...
export const errorLensTools = [
  {
    ...
    description:
      'Get all diagnostics (errors, warnings, info) from the current project',
    inputSchema: {
      type: 'object',
      properties: {
        severity: {
    ...
    handler: async (args: any) => {
      try {
        ...
        try {
          const { stdout: tscOutput } = await execa(
            'npx',
        ...
        try {
          const { stdout: eslintOutput } = await execa(
            'npx',
            ['eslint', '.', '--format', 'json'],
            {
              reject: false,
  ...
  },
  {
    ...
    handler: async () => {
      try {
        ...
        if (tscOutput) {
          ...
          for (const line of lines) {
            ...
            if (match) {
              ...
              if (!(summary.byFile as any)[fileName]) {
                (summary.byFile as any)[fileName] = { errors: 0, warnings: 0 };
              }

              if (severity === 'error')
                (summary.byFile as any)[fileName].errors++;
              else if (severity === 'warning')
                (summary.byFile as any)[fileName].warnings++;
            }
          }
        }

        // Get top issues
        const fileList = Object.entries(summary.byFile as Record<string, any>)
          .sort(
            (a, b) =>
        ...
        (summary as any).topIssues = fileList.map(
          ([file, counts]: [string, any]) => ({
            file,
            ...counts,
          })
        );
