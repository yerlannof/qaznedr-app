  ...
  const baseRecommendations = [
    {
      ...
      description: 'Перспективное золотое месторождение с подтвержденными запасами и современной инфраструктурой.',
      price: 2500000000,
      area: 850,
      mineral: 'Gold',
      region: 'Актюбинская',
    ...
    },
    {
      ...
      description: 'Высокопотенциальный участок для нефтедобычи в перспективном регионе.',
      price: 8900000000,
      area: 2400,
      mineral: 'Oil',
    ...
    },
    {
      ...
      description: 'Крупное медное месторождение с развитой логистической инфраструктурой.',
      price: 4200000000,
      area: 1650,
      mineral: 'Copper',
        ...
        <div className="flex items-center justify-between mb-3">
          <motion.div 
            className={`inline-flex items-center space-x-2 px-3 py-1 rounded-full text-xs font-medium ${
              reason.color === 'blue' ? 'bg-blue-50 text-blue-700' :
              reason.color === 'red' ? 'bg-red-50 text-red-700' :
              reason.color === 'purple' ? 'bg-purple-50 text-purple-700' :
              reason.color === 'green' ? 'bg-green-50 text-green-700' :
...
export default function Recommendations({ 
  ...
  const [recommendations, setRecommendations] = useState<Array<KazakhstanDeposit & { reason: string; score: number }>>([]);
    ...
    return (
      <div className={`space-y-6 ${className}`}>
        <div className="flex items-center space-x-2">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Рекомендации для вас</h2>
        </div>
