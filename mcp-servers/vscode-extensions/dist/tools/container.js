...
export const containerTools = [
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                features: {
                    type: "array",
                    items: { type: "string" },
                    description: "Additional features to include (docker-in-docker, git, etc.)",
                },
            },
            required: ["type"],
        },
        handler: async (args) => {
            ...
            if (args.features?.includes("docker-in-docker")) {
                devcontainerJson.features["ghcr.io/devcontainers/features/docker-in-docker:2"] = {};
            }
            ...
            switch (args.type) {
                ...
                case "python":
                    ...
                    devcontainerJson.postCreateCommand = "pip install -r requirements.txt";
            ...
            await fs.writeFile(path.join(devContainerPath, "devcontainer.json"), JSON.stringify(devcontainerJson, null, 2));
...
`;
            ...
            return {
                ...
                instructions: "Reopen in Container using Command Palette (F1) > 'Dev Containers: Reopen in Container'",
            };
        },
    },
    {
        ...
        handler: async () => {
            ...
            if (status.isContainer) {
                try {
                    ...
                    status.containerInfo = { hostname: hostname.trim() };
                }
                catch { }
            }
            // Check if in WSL
            ...
            if (process.env.VSCODE_REMOTE_CONTAINERS) {
                ...
                status.remoteInfo = { type: "container" };
            }
            else if (process.env.VSCODE_REMOTE_WSL) {
                status.isRemote = true;
                status.remoteInfo = { type: "wsl" };
            }
            else if (process.env.VSCODE_REMOTE_SSH) {
                status.isRemote = true;
                status.remoteInfo = { type: "ssh" };
            }
            return status;
        },
    },
    {
        ...
        handler: async (args) => {
            switch (args.action) {
                case "list":
                    const { stdout: volumes } = await execa("docker", ["volume", "ls", "--format", "json"]);
                    return volumes.split("\n").filter(Boolean).map(line => JSON.parse(line));
                case "inspect":
                    if (!args.name)
                        throw new Error("Volume name required");
                    const { stdout: info } = await execa("docker", ["volume", "inspect", args.name]);
                    return JSON.parse(info);
                case "create":
                    if (!args.name)
                        throw new Error("Volume name required");
                    const { stdout: created } = await execa("docker", ["volume", "create", args.name]);
            ...
            try {
                switch (args.action) {
                    case "context":
                        ...
                        const { stdout: contexts } = await execa("kubectl", ["config", "get-contexts"]);
                        return { current: context.trim(), all: contexts };
                    case "pods":
                        const { stdout: pods } = await execa("kubectl", ["get", "pods", "-n", namespace, "-o", "json"]);
                        return JSON.parse(pods);
                    case "services":
                        const { stdout: services } = await execa("kubectl", ["get", "services", "-n", namespace, "-o", "json"]);
                        return JSON.parse(services);
                    case "deployments":
                        const { stdout: deployments } = await execa("kubectl", ["get", "deployments", "-n", namespace, "-o", "json"]);
