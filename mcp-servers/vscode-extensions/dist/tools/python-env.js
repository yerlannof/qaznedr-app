...
export const pythonEnvTools = [
    {
        ...
        handler: async () => {
            ...
            try {
                ...
                const { stdout: pythonVersion } = await execa(".venv/bin/python", ["--version"]);
                environments.push({
                    type: "venv",
                    path: ".venv",
            ...
            try {
                const { stdout: condaEnvs } = await execa("conda", ["env", "list", "--json"]);
    ...
    },
    {
        ...
        handler: async (args) => {
            ...
            return {
                ...
                instruction: "Run this command in your terminal to activate the environment",
                vscodeSettings: {
                    "python.defaultInterpreterPath": args.type === "venv"
                        ? `${args.path}/bin/python`
                        : args.path,
                },
            };
        },
    },
    {
        ...
        handler: async (args) => {
            ...
            if (args.requirements) {
                const { stdout, stderr } = await execa("pip", ["install", "-r", args.requirements]);
    ...
    },
    {
        ...
        handler: async (args) => {
            if (args.outdated) {
                const { stdout } = await execa("pip", ["list", "--outdated", "--format", "json"]);
